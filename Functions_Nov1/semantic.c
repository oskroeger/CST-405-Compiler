...
void semanticCheck(ASTNode* node, SymbolTable* symTab) {
    ...
    switch (node->type) {
        ...
        case NodeType_Block:
            semanticCheck(node->block.varDeclList, symTab);
            semanticCheck(node->block.stmtList, symTab);
            break;  // Added for function blocks

        case NodeType_ParamList:
            semanticCheck(node->ParamList.Param, symTab);
            semanticCheck(node->ParamList.ParamList, symTab);
            break;
        
        case NodeType_Param:
            printf("Checking parameter '%s' of type '%s'...\n", node->Param.varName, node->Param.varType);
            break;
