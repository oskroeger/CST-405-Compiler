...
void printTAC() {
    TAC* current = tacHead;
    while (current != NULL) {
        if (strcmp(current->operation, "FUNC_START") == 0) {
            // Function start marker
            printf("function %s START\n", current->result);
        } else if (strcmp(current->operation, "FUNC_END") == 0) {
            // Function end marker
            printf("function %s END\n", current->result);
        } else if (strcmp(current->operation, "CALL") == 0) {
            // Function call
            printf("call %s\n", current->result);
            ...
            int isFloatOp1 = isFloatOperand(current->operand1);
            int isFloatOp2 = isFloatOperand(current->operand2);

            if (isFloatOp1 && isFloatOp2) {
                // Both operands are float numbers
                printf("%s = %.4f %s %.4f\n", current->result, atof(current->operand1), current->operation, atof(current->operand2));
            } else if (isFloatOp1) {
                // Operand1 is float, Operand2 is not
                printf("%s = %.4f %s %s\n", current->result, atof(current->operand1), current->operation, current->operand2);
            } else if (isFloatOp2) {
                // Operand1 is not float, Operand2 is
                printf("%s = %s %s %.4f\n", current->result, current->operand1, current->operation, atof(current->operand2));
            } else {
