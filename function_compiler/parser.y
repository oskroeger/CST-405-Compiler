...
%type <ast> Program VarDecl VarDeclList Stmt StmtList Expr ArrayAccess FuncDecl FuncDeclList 
%start Program
...
FuncDeclList:
    /* empty */ {
        $$ = NULL; // Handle case where there are no function declarations
    }
    | FuncDecl FuncDeclList {
        ...
        printf("[INFO] Function declaration list recognized.\n");
    }
;

FuncDecl:
    TYPE ID LPAREN VarDeclList RPAREN LCURLY VarDeclList StmtList RCURLY {
        $$ = createNode(NodeType_FuncDecl);
        $$->funcDecl.funcName = strdup($2);
        $$->funcDecl.paramList = $4;
        $$->funcDecl.varDeclList = $7;  // Local variable declarations
        $$->funcDecl.stmtList = $8;     // Statements inside the function

        addFunctionSymbol(symTab, $2, $1, $4);
        printf("[INFO] Function declared: %s\n", $2);
    }
;
